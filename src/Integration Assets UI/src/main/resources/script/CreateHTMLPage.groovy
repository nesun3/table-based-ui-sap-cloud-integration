...
def Message processData(Message message) {
    ...
    if (filterBy.toString().toUpperCase() == "CONF") {
        List configurationsAll = prop.get("configurationsAll")
        listOfMaps = configurationsAll
        name = "Configurations"
        pageTitle = "All Integration $name"
    }
    else {
        List resourcesAll = prop.get("resourcesAll")
        listOfMaps = resourcesAll
        name = "Resources"
        pageTitle = "All Integration $name"
    }
...
//                link(rel: "stylesheet", type: "text/css", href: "https://cdn.datatables.net/1.13.6/css/jquery.dataTables.min.css")
            ...
            body {
                ...
                \$(document).ready(function() {
                     document.title = "Integration $name " + new Date().toISOString().slice(0, 19).replaceAll(':','');
                    \$('#example').DataTable( { 
                        dom: 'Bifrtlp',
                        fixedHeader: true,
                        fixedColumns:{
        ...
        message.setBody("$name returns no result")
